...
The gate verifies the Groth16 proof on-chain and marks your account eligible. This survives future root rotations; you never re-prove.
...
- **Custom source (your code):** implement one adapter function that returns `{ period, value, payload }` for your source, following `agent/src/eia.ts` as the template.
...
PAYEE_ADDRESS=<your public key hex> \
