...
<Callout type="info">
**Hosted base URL:** `https://claros-oracle.vercel.app`. When you self-host the service it listens on `http://localhost:4030` by default. Everything points at Casper **testnet** (network id `casper-test`); reads need no key and no wallet.
</Callout>
...
Host: claros-oracle.vercel.app
```

```bash
curl https://claros-oracle.vercel.app/v1/feeds/EIA.PET.PRICE.WTI.DAILY
```
...
List every feed that is live on-chain, each as a full reading (metadata plus latest value). There are **38 feeds** live today. There are no parameters and no pagination: the call enumerates all feed ids, then reads each one, so it is the heaviest endpoint. To read a single value, prefer [`GET /v1/feeds/:id`](#get-v1feedsid).

```bash
curl https://claros-oracle.vercel.app/v1/feeds
```
...
```bash
curl "https://claros-oracle.vercel.app/v1/datasets?family=electricity&q=price"
```
...
```bash
curl https://claros-oracle.vercel.app/health
```
