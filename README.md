...
4. **Earn.** The operator runs the same confirmed feed updater with its own key and feed config (`agent/operator-feeds.json` and `agent/feed-operator2.json` are the working configs operator #2 uses) and sells reads over the same x402 rail. Operator #2 was [settled 1 WCSPR](https://testnet.cspr.live/transaction/65534cbb1351abdbdbe27298580772ee1a74c8ac696a8e6ace0209ab3b2e3ce4) for a read of its solar feed.
